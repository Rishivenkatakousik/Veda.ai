...
export const buildUserPrompt = (input: PromptInput): string => {
  ...
  lines.push(
    ``,
    `Requirements:`,
    `- Create exactly one JSON section per line in "Question breakdown" above, in the same order (first row → sections[0], second row → sections[1], etc.).`,
    `- Each section must contain exactly the number of questions listed for that row.`,
    `- For any row whose type is Multiple Choice (or MCQ): every question in that section MUST include an "options" array with exactly four strings (the four answer choices).`,
    ...
    `- Use clear section titles and instructions (mention multiple choice where applicable).`,
