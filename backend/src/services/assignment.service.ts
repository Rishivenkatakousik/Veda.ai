...
export const createAssignment = async (input: CreateAssignmentInput): Promise<{
    ...
    const job = await assignmentQueue.add("assignment-generate", {
...
export const regenerateAssignment = async (id: string, options?: {
    correlationId?: string;
}): Promise<{
    assignment: AssignmentDocument;
    jobId: string;
...
export const softDeleteAssignment = async (id: string) => {
    return AssignmentModel.findOneAndUpdate({ _id: id, isDeleted: false }, { isDeleted: true, deletedAt: new Date() }, { new: true }).lean();
};
